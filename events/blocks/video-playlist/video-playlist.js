...
class VideoPlaylist {
    ...
    // ... (All existing helper methods like _filterCards, _sortCards, _displayPlaylist, 
    // _createHeader, _createSocialSharingButton, _generateSocialMenuItems, 
    // _setupAutoplayCheckbox, _setupSocialSharing, _handleShare, 
    // _copyToClipboard, _fallbackCopyToClipboard, _openShareWindow, 
    // _showNotification (for copy link), _createSessionsWrapper, 
    // _setInitialProgressBars are retained from the first portion.)

    /* --- Header and Sharing (Retained from Portion 1) --- */
    ...
    _generateSocialMenuItems() {
      ...
      Object.keys(platforms).forEach(platformKey => {
          ...
          if (this.cfg[platformData.enableKey]) {
              ...
              const href = platformKey === 'copy' ? '#' : platformData.href || '#'; 
              menuHtml += `
    ...
    // ... (All other social sharing and copy-link helpers are implicitly here)

    /* --- Sessions List (Retained from Portion 1) --- */
    ...
    /* --- Favorites (Refactored Notification & Toggle Logic) --- */
    ...
    _createFavoriteButton(card, isFavorite) {
        ...
        // Use configured text for the 'Add' state tooltip, or 'Remove' if already favorited
    ...
    /* --- Video Player Integration (Refactored) --- */
    ...
    _watchForVideoContainer() {
        ...
        // Watch for the .milo-video element to be added to the DOM
        observer.observe(document.body, { childList: true, subtree: true });
    }

    _setupPlayerListeners() {
        this._highlightCurrentSession();
        // Remove and re-add message listener to prevent duplicates
    ...
    _handlePlayerMessage(event) {
        // Handle MPC message for progress/completion
    ...
    _handleMpcLoad(data) {
        ...
        // Send a command to the MPC player iframe to start from the saved second
    ...
    _handleVideoComplete(videoId) {
        ...
        if (shouldPlayAll) {
            ...
            if (currentSessionIndex !== -1 && currentSessionIndex < this.cards.length - 1) {
                ...
                // Navigate to the next session's page
                window.location.href = nextSessionURL.href;
            }
        }
    }

    /* --- YouTube Integration (New Logic) --- */

    _setupYouTubePlayer() {
        // Load YouTube IFrame API script only if it's not present
        if (!window.YT) {
        ...
        // Set the callback once the API script is loaded
    ...
    _setupLiteYouTubePlayer(liteYoutube, videoId) {
        // Wait for the lite-youtube element to be clicked and replaced with a real iframe
        const clickHandler = () => {
    ...
    _addEnableJsApiToIframe(iframe, videoId) {
        try {
            ...
            if (currentSessionData && currentSessionData.secondsWatched > RESTART_THRESHOLD) {
                // Set the start time if progress is significant
                url.searchParams.set('start', Math.floor(currentSessionData.secondsWatched));
            }

            iframe.src = url.toString();
            // Once the iframe reloads with the API parameters, start tracking
    ...
    _setupYouTubeProgressTracking(iframe, videoId) {
        if (window.YT && window.YT.Player) {
            ...
            // Fallback: wait for the YT API to load if initialization missed it
    ...
    _startYouTubeProgressTracking(player, videoId) {
        ...
        // Track progress every 1 second (per PROGRESS_SAVE_INTERVAL logic)
    ...
    async _recordYouTubePlayerProgress(player, videoId) {
        try {
            ...
            if (currentTime && duration) {
                // Save progress only if time is a multiple of the interval
    ...
    _updateProgressBarForVideo(videoId, currentTime, duration) {
        ...
        if (sessionElement) {
            ...
            if (progressBar) {
                ...
                progressBar.style.width = `${Math.min(100, progress)}%`;
                progressBar.style.backgroundColor = '#1473e6'; // Ensure progress bar is visible blue
    ...
    static dispose() {
        // Not typically used in production, but good practice for testing/cleanup
